...
class DNNSequentialRecommender(Recommender):
    ...
    def add_action(self, action):
        ...
        self.user_actions[user_id_internal].append((action.timestamp, action_id_internal, int(action.data['skipped'])))
    ...
    def train_epoch_debug(self, generator, val_generator):
        ...
        metric_sum = 0
        num_batches = 0
        for X, y_true in pbar: # X : embeddings
            num_batches += 1
            with tf.GradientTape() as tape:
                y_pred = self.model(X, training=True)
                # LOSS VAL BELOW IS SIMPLY MeanPredLoss()
