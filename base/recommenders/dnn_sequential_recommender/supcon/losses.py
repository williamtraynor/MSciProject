...
def contrastive_loss(features,
  ...
  #print(f'DIAGONAL MASK SHAPE: {diagonal_mask.shape}\nDiagonal Mask: {diagonal_mask}')
  ...
  #print('POSTIVE + NEGATIVE MASKS')
  #print(positives_mask, negatives_mask)
...
'''def contrastive_loss_alt(features,
                     labels=None,
                     temperature=1.0,
                     contrast_mode=enums.LossContrastMode.ALL_VIEWS,
                     summation_location=enums.LossSummationLocation.OUTSIDE,
  ...
  local_batch_size, num_songs, num_views = labels.shape
  ...
  print('LOCAL Replica ID: ', local_replica_id)

  # Generate the [local_batch_size, global_batch_size] slice of the
  # [global_batch_size, global_batch_size] identity matrix that corresponds to
  # the current replica.
  #diagonal_mask = tf.one_hot(
  #    tf.range(local_batch_size) + (local_replica_id * local_batch_size), num_songs,
  #    local_batch_size)

  #print('Diag Mask Shape: ', diagonal_mask.shape, diagonal_mask)
  ...
  if labels is None:
    # Defaults to every sample belonging to its own unique class, containing
    # just that sample and other views of it.
    mask = None # diagonal_mask Nor required - we will always have labels
  ...
  if contrast_mode == enums.LossContrastMode.ONE_VIEW:
    anchor_features = features[:, :, -1] # take last feature as it is definitely not a pad
    num_anchor_views = 1
  else:  # contrast_mode == enums.LossContrastMode.ALL_VIEWS
    # Reshape features to match how global_features is reshaped above.
    ...
    #anchor_features = tf.reshape(
    #    tf.transpose(features, perm=[1, 0, 2]),
    #    [num_songs * local_batch_size, -1])
    #num_anchor_views = num_songs

    print('Anchor Features Shape: ', anchor_features.shape)
  ...
  # just for numerical precision.
  logits = tf.linalg.matmul(
      anchor_features, anchor_features, transpose_b=True)
  temperature = tf.cast(temperature, tf.float32)
  logits = logits / temperature
  logits = (
      logits - tf.reduce_max(tf.stop_gradient(logits), axis=1, keepdims=True))
